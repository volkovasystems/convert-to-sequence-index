...
( function module( base ){
	...
	*/
	moduleDefine( "convertToSequenceIndex",
		function construct( ){
			var childprocess = require( "child_process" );
			var fs = require( "fs" );
			var path = require( "path" );
			var pathDirectory = process.cwd( ) + "/convert-to-sequence-index/".replace( "/", path.sep );
			var javaClassFile = pathDirectory + "convertToSequenceIndex.class";
			var isJavaClassFileExist = fs.existsSync( javaClassFile );

			var convertToSequenceIndex = function convertToSequenceIndex( separator, sequence, dictionary, callback ){
				var parameters = Array.prototype.slice.apply( arguments );
				...
				var command = "java " + pathDirectory + "convertToSequenceIndex " + numbers.join( " " );	
				if( !isJavaClassFileExist ){
					command = "javac " + pathDirectory + "convertToSequenceIndex.java && " + command;
				}
			...
			base.convertToSequenceIndex = convertToSequenceIndex;
		} );
} )( base );
